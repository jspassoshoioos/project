...
cta2 %>% filter(stationname == "Cermak-Chinatown", date > as.Date("2006-12-01") & date < as.Date("2008-11-01")) %>% arrange(date) %>% print(n = Inf)
```

The specific month for beginning of Addison project was unavailable, all sources 
we could find referencing the project indicated that construction "began in Fall 2015," 
...
I am unable to join all of the data together in one pipe so I have to add one each time.
```{r joining data sets}
join <- 
...
Here we can see that there was notable growth between before the reconstruction and after, 
but how did the system compare for the same time-period? 
...
a  semitrailer truck came speeding off the exit ramp and collided into the station 
...
Issue here is the drastically different y scales (almost a 100 fold difference), when compared
side by side it makes it seem that the totals are similar, even though they are drastically different.
...
of Rodgers Park. On the opposite side of the city, Garfield is located in the South side
...
```{r Charts, fig.width = 30, eval = F, include = F}
