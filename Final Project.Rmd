---
title: "PA 434 Final Project - Messy"
...
the many neighborhoods and communities within the city. It is also known that Chicago is a segregated city. Decades of practices such as redlining, restrictive housing covenants, as well as outright legalized housing discrimination has left the city with stark racial and ethnic boundaries that remain clear even
...
[Congressional districts](https://upload.wikimedia.org/wikipedia/commons/thumb/b/be/United_States_Congressional_Districts_in_Illinois_%28metro_highlight%29%2C_2003_%E2%80%93_2013.tif/lossless-page1-1226px-United_States_Congressional_Districts_in_Illinois_%28metro_highlight%29%2C_2003_%E2%80%93_2013.tif.png) take particularly unusual form in an effort to follow these tight boundaries. 

Acknowledging the extent of the city's segregated state is critical, as even the 
wishful or naive supposition that the city is not segregated can have profound
consequences on the communities that have been historically marginalized and denied
equal opportunities and access. The original motivation for this research was 
to study if there *was* bias in determining the areas in which Chicago Transit Authority (CTA) 
was making improvements (e.g., improvements being made solely or predominately in White, wealthy, 
well educated neighborhoods). Through initial research of this topic, the answer to 
that question became clear: there is indeed bias in the investments the CTA makes into 
improvements, but these are inherent biases that are the result of the decades of institutional racism and 
bias towards communities of color. 

Consider the shape of the CTA system, sprawling with many lines radiating outward
from the central downtown Loop. Lines disseminate in every direction 
...
are equally divided heading South (3) and West (3). Looking at a demographic 
...
and West sides, the lines take on much different shapes, with two lines, the Red (Dan-Ryan Branch) and Blue line (Forest Park Branch), being constructed in the medians of the interstate highways that were constructed during the post-war housing boom. As many White households moved out of city areas in favor of suburban living, a phenomenon often refer to as "White flight," the investment and focus on creating reliable transportation infrastructure followed them to the suburbs. 

Thus it became clear that if our research was seeking to determine if there is in fact
a bias in the areas served by and thus the recipient of transportation improvement projects, that
...
can be more readily addressed and the historical wrongs to minority communities can be made right. 
...
effect would we expect these projects to have on ridership within these communities? 
     + In a system that was made to serve primarily White, middle to upper class, primarily English speaking 
 and highly educated areas, we would expect improvements to cluster in these communities and to result in a greater
...
To study the demographic data of the areas subject to CTA improvements, we  utilize data made available by the [American Community Survey (ACS)](https://www.census.gov/programs-surveys/acs/about.html), a demography 
...
aligning with the community area boundaries. Within the demographic data, we are interested in the following variables:
...
from 2005 to 2010. This decision was driven  Although these estimates will be frequently referred to as the **2010 estimates** or **2010 5 year estimates** the estimate values are reflective of the estimated value for the entire 5 year period (i.e., 2005 to 2010) not simply the 2010 calendar year.
...
were determined from [this map](https://www.csu.edu/cerc/documents/ChicagoCommunityAreasMapWithCensusTracts2000-NIPC.pdf) published by the Northeastern Illinois Planning Commission. What quickly became apparent was there was some data missing. The 2010 census found 
a total population for the city of Chicago to be approximately 2.6 million, while 
our the aggregate of our census data indicates a population estimate of approximately 2.4 
million. 

To identify the specific improvement projects of interest, the [CTAs list](https://www.transitchicago.com/newsprojects/system-improvement-projects/completed-station-projects) of past projects was reviewed. From this list a deliberate sample was selected with the intent to ensure 
that the group of improvements was a heterogeneous group with demographics comparable to the 
city at large. The goal of this is to then compare the differences in ridership from before and after the improvement was made to see if there were any meaningful differences across the different groups, for example, if projects in predominantly White neighborhoods have a greater impact on ridership than those made in predominately minority neighborhoods. 

To see if there were any meaningful changes in the ridership before and after the improvement was made, the CTA's publicly available [daily ridership dataset](https://data.cityofchicago.org/Transportation/CTA-Ridership-L-Station-Entries-Daily-Totals/5neh-572f) that provides the daily number of station entries for each station from 2001 to present is used. We plan to visualize the ridership 
data from before and after the various improvement projects were made to see if there are 
any discernible trends that could indicate a correlation between our demographic variables
and the impact improvement projects have on ridership. 
...
string for you to insert your own API key. 
...
and the data and variables accessible from said APIs, and can be quite useful when
...
project_geo <- listCensusMetadata(name = "acs/acs5", vintage = 2010, type = "geography")
...
community areas related to our improvement projects, we then used the identified census 
tracts to calculate the approximate populations of each community.
...
For our race and ethnicity variable, we use the various racial/ethnic groups utilized by the Census. To gauge 
level of education we have broken neighborhoods down into three categories, less than a high school 
degree, a high school degree and up to some college, and college graduates. The census also collects
estimates regarding the various languages spoken by individuals. Given the wide range of language
options that the census has available, we have limited our review to Chicago's top six languages: 
...
One variable we encountered some difficulty with was income. Because most of the census' estimates
are point estimates for the number of individuals of a certain category (i.e., number of Black males, English speaking Asians, etc.)
the only meaningful income variable that could be found was the calculated median income of the given 
census tract. In order to create a meaningful value that reflects the income of the overall neighborhood, 
we summed the median incomes of each neighborhood and then calculated the mean of those median incomes, resulting
in the mean median income, or the average median income for the neighborhood. 
...
percents$neighborhood <- replace(percents$neighborhood, 8, "Areas with CTA Impovements")
```
...
```
Before proceeding with further analysis into the various language groups, we did 
want to first check the data to see if there was any values that stood out. We expect 
that the top two languages will be English then Spanish and that these will likely 
be the majority of languages spoken (except in Armour Square where we expect most 
residents to be Chinese speaking). 

Upon review, the only notable observation is that 6% of Avondale, a predominately Hispanic community, 
are Polish speakers and this is notably higher than the city-wide average. All other values are what we would expect
(high amount of Chinese spoken in Armour Square due to Chinatown being located there) and Avondale being primarily Spanish
speakers while Englewood and Rosewood are almost exclusively English speakers (likely due to the fact that 98% of the community
is African American/Black). Also Near North Side is predictably higher than city average for English, given that this 
...
After preparing and cleaning our demographic data, we were able to calculate the
following demographic information, presented below in tables (one table per demographic 
variable). **Please note:** in addition to reporting the individual neighborhoods 
where improvements were made, an aggregate row for all areas where CTA improvement 
projects were taking place so that the aggregate area can be compared to the city-wide
demography to see if there are any notable discrepencies between the areas where
CTA projects were being carried out and the rest of the city. 


```{r Table of Race Data, include=TRUE}
...
```{r Table of Lang Data, include=TRUE}

percents  %>%
  select(neighborhood,  native_gd_per:primary_sp) %>%
  kable(col.names = c("Area",
                      "English", "Non-Native English", "Primary Not Eng", "Spanish"), 
        align = "ccc", 
...
percents  %>%
  ...
  kable(col.names = c("Area",
                      "Mean Median Income*"), 
        align = "ccc", 
        caption = "Average Median Income of Neighborhoods where CTA Improvements were Made, and City-Wide Values",
        digits = 2, 
        format.args = list(big.mark = ",")) %>%
  kable_styling(latex_options = c("striped", "scale_down")) %>%
...
As previously noted, the City of Chicago's population in 2010 was 2.6 million, and 
our data indicates an estimate of approximately 2.4 million so we are fairly close,
a tract or two along the edges may have been excluded, but the bulk of the city's population 
...
Now that we have seen the demographics of the areas that received improvements, we 
can now move to plots of the change in ridership, including both the total change 
in ridership as well as the change as a percentage of the difference between the 
mean number of riders before and after the improvements were made. 

```{r themes} 

red_line <- theme_minimal() + 
  theme(plot.title = element_text(color = "#c60c30", size = 14, face = "bold"), 
...
First, we will look at the data for Cermak-Chinatown. 
...
plot_4 <- ggplot() +
  geom_hline(yintercept = mean(Cer_Chi$perc_mean), color = "#c60c30", alpha = 0.5, linetype = "twodash") + # Include average lines to help see average despite variation 
  geom_hline(yintercept = mean(System_Cermak$perc_mean), color = "Black", alpha = 0.5, linetype = "twodash") +
...
and after the accident (i.e., February 2008 to February 2009), then we should be able 
to observe a notable drop off in response to the accident: 

```{r 2008 down trend, include = T}

plot_4.1 <- ggplot() +
  geom_hline(yintercept = mean(Cer_Chi2$Cer_Chi_bef), color = "#c60c30", alpha = 0.5, linetype = "twodash") + 
...
plot_4.2 <- ggplot() + 
  geom_hline(yintercept = mean(System_Cermak2$before_mean), color = "Black", alpha = 0.5, linetype = "twodash") + 
...
```{r, include = T}
ggplot() +
  geom_hline(yintercept = mean(Cer_Chi2$perc_mean), color = "#c60c30", alpha = 0.5, linetype = "twodash") + # Include average lines to help see average despite variation 
  geom_hline(yintercept = mean(System_Cermak2$perc_mean), color = "Black", alpha = 0.5, linetype = "twodash") +
...
```{r Cermak vs System test, include = T}
...
a <- ggplot() +
  geom_hline(data = Cer_Chi, aes(yintercept = mean(Cer_Chi$Cer_Chi_bef), color = "#c60c30"), alpha = 0.5, linetype = "twodash") + 
  geom_hline(data = DanRyan, aes(yintercept = mean(Cer_Chi$Cer_Chi_aft), color = "#f598a9"), alpha = 0.5, linetype = "twodash") +
  ...
  geom_line(data = Cer_Chi, aes(x = month, y = Cer_Chi_aft, group = 1), 
            color = "#f598a9") +
  geom_point(data = Cer_Chi, aes(x = month, y = Cer_Chi_aft, group = 1, color = "#f598a9"))  + 
  scale_color_manual(name = "Time", 
                    labels = c("Before", "After"), 
                    values = c("#c60c30", "#f598a9")) +
  labs(title = "Cermak-Chinatown") +
  xlab("Number of Months Before/After Project") + 
  ylab("Mean # of Riders") +
  red_line
...
b <- ggplot() +
  geom_hline(data = System_Cermak, aes(yintercept = mean(System_Cermak$before_mean), color = "Black"), alpha = 0.5, linetype = "twodash") + 
  geom_hline(data = System_Cermak, aes(yintercept = mean(System_Cermak$after_mean), color = "Gray"), alpha = 0.5, linetype = "twodash") +
...
```{r DanRyan vs System, include=T}
plot_7 <- ggplot() +
  geom_hline(yintercept = 0, color = "Gray", linetype = "dashed") +
  geom_hline(yintercept = mean(DanRyan$perc_mean), color = "#c60c30", alpha = 0.5, linetype = "twodash") +
  geom_hline(yintercept = mean(System_DanRyan$perc_mean), color = "Black", alpha = 0.5, linetype = "twodash") +
  geom_line(data = DanRyan, aes(x = month, y = perc_mean, 
            group = 1, color = "#c60c30")) +
  geom_point(data = DanRyan, aes(x = month, y = perc_mean, group = 1, color = "#c60c30")) +
  geom_line(data = System_DanRyan, aes(x = month, y = perc_mean), 
            group = 1, color = "Black") +
  geom_point(data = System_DanRyan, aes(x = month, y = perc_mean, color = "Black", 
            group = 1)) +
  ...
  ylab("% Change in Mean Ridership") +
  scale_color_manual(name = "Station", 
                    labels = c("95th/Dan Ryan (Red Line)", "System-wide"), 
                    values = c("#c60c30", "Black")) +
  red_line
...
```{r DanRyan B&A Total rides, include =T}
plot_7.1 <- ggplot() +
  geom_hline(data = DanRyan, aes(yintercept = mean(DanRyan$Dan_bef), color = "#c60c30"), alpha = 0.5, linetype = "twodash") + 
  geom_hline(data = DanRyan, aes(yintercept = mean(DanRyan$Dan_aft), color = "#f598a9"), alpha = 0.5, linetype = "twodash") +
  geom_line(data = DanRyan, aes(x = month, y = Dan_bef, group = 1, color = "#c60c30")) +
  geom_point(data = DanRyan, aes(x = month, y = Dan_bef, group = 1, color = "#c60c30")) +
  geom_line(data = DanRyan, aes(x = month, y = Dan_aft),group = 1, color = "#f598a9") +
  geom_point(data = DanRyan, aes(x = month, y = Dan_aft, group = 1, color = "#f598a9")) + 
  scale_color_manual(name = "Time", 
                    labels = c("Before", "After"), 
                    values = c("#c60c30", "#f598a9")) +
  labs(title = "95th/Dan Ryan") +
  xlab("Number of Months Before/After Project") + 
  ylab("Mean # of Riders") +
  red_line
...
plot_7.2 <- ggplot() + 
  geom_hline(data = System_DanRyan, aes(yintercept = mean(System_DanRyan$before_mean), color = "Black"), alpha = 0.5, linetype = "twodash") + 
  geom_hline(data = System_DanRyan, aes(yintercept = mean(System_DanRyan$after_mean), color = "Gray"), alpha = 0.5, linetype = "twodash") + 
  geom_line(data = System_DanRyan, aes(x = month, y = before_mean, group = 1, color = "Black")) +
  geom_point(data = System_DanRyan, aes(x = month, y = before_mean, group = 1, color = "Black")) +
geom_line(data = System_DanRyan, aes(x = month, y = after_mean, group = 1, color = "Gray")) +
  geom_point(data = System_DanRyan, aes(x = month, y = after_mean, group = 1, color = "Gray")) +
   scale_color_manual(name = "Time", 
                    labels = c("Before", "After"), 
                    values = c("Black", "Gray")) + 
  labs(title = "System-Wide") +
...
figure_2 <- ggarrange(plot_7.1, plot_7.2, nrow = 1, ncol = 2)
...
```{r Jarvis vs System, include=T}
plot_1 <- ggplot() +
  geom_hline(yintercept = 0, color = "Gray", linetype = "dashed") +
  geom_hline(data = Jarvis, aes(yintercept = mean(Jarvis$perc_mean), color = "#c60c30"), alpha = 0.5, linetype = "twodash") +
  geom_hline(data = System_Jarvis, aes(yintercept = mean(System_Jarvis$perc_mean), color = "Black"), alpha = 0.5, linetype = "twodash") +
  geom_line(data = Jarvis, aes(x = month, y = perc_mean, group = 1, color = "#c60c30")) +
  geom_point(data = Jarvis, aes(x = month, y = perc_mean, group = 1, color = "#c60c30")) +
  geom_line(data = System_Jarvis, aes(x = month, y = perc_mean, group = 1, color = "Black")) +
  geom_point(data = System_Jarvis, aes(x = month, y = perc_mean, group = 1, color = "Black")) +
  scale_y_continuous(limits = c(-20,15)) +
  labs(title = "Change in Ridership at Jarvis vs. System Wide",
       subtitle = "Difference in the average number of riders for the 12 month period before November 2012 and after December 2012",
       caption = "Source: Chicago Transit Authority Daily Ridership Dataset via Chicago Data Portal") +
  xlab("Number of Months Before/After Project") + 
  ylab("% Change in Mean Ridership") +
  scale_color_manual(name = "Station", 
                    labels = c("Jarvis (Red Line)", "System-wide"), 
                    values = c("#c60c30", "Black")) +
  red_line
...
```{r Garfield vs System, include=T}
plot_6 <- ggplot() +
  geom_hline(yintercept = 0, color = "Gray", linetype = "dashed") +
  geom_hline(data = Garfield, aes(yintercept = mean(Garfield$perc_mean)), color = "#c60c30", alpha = 0.5, linetype = "twodash") +
  geom_hline(data = System_Garfield, aes(yintercept = mean(Garfield$perc_mean)), color = "Black", alpha = 0.5, linetype = "twodash") +
  geom_line(data = Garfield, aes(x = month, y = perc_mean, group = 1, color = "#c60c30")) +
  geom_point(data = Garfield, aes(x = month, y = perc_mean, group = 1, color = "#c60c30")) +
  geom_line(data = System_Garfield, aes(x = month, y = perc_mean), group = 1, color = "Black") +
  geom_point(data = System_Garfield, aes(x = month, y = perc_mean, color = "Black"), group = 1) +
  scale_y_continuous(limits = c(-20,20)) +
  labs(title = "Change in Ridership at Garfield vs. System Wide",
       subtitle = "Difference in the average number of riders for the 12 month period before May 2013 and after October 2013",
       caption = "Source: Chicago Transit Authority Daily Ridership Dataset via Chicago Data Portal") +
  xlab("Number of Months Before/After Project") + 
  ylab("% Change in Mean Ridership") +
  scale_color_manual(name = "Station", 
                    labels = c("Garfield (Red Line)", "System-wide"), 
                    values = c("#c60c30", "Black")) +
  red_line
...
Our findings show very little growth in ridership above system-wide trends
following improvements to our seven selected stations. The increase in ridership
at the Cermak-Chinatown station is slightly higher than system-wide trends though
this may also by due to external factors such as recovery of ridership after the 
truck incident that made station improvements necessary. Other increases were 
minor and in neighborhoods where increase would be predicted based on demographics. 
The most unexpected result, the steep decline in ridership experienced by 95th/Dan Ryan 
is actually in line with the theoretical framework that improvements in minority 
communities essentially amount to lip service, if not actively harming said communities. 

Ultimately these data show that more research could be done to understand ridership
trends, particularly in neighborhoods largely comprised of people of color. As we show, 
ridership after the 95th/Dan Ryan station project drastically decreased, with the Garfield
station seeing a slight decrease in ridership trends as well, though less dramatic. Future
projects could explore whether bus ridership from these neighborhoods increased over 
the course of improvement projects and whether trends perhaps remained constant after 
project completion.
